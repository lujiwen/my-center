...
{
   public  class DeviceUIKSJKSD : DeviceUI 
    {
       static int x = 1;
       DeviceDataBox_KSJKSD box;
       LabelAndText stateLT;
       LabelAndText valueLT;
       private DeviceKSJKSD device;
       public DeviceUIKSJKSD(DeviceKSJKSD d, Frame fm)
         :base(d, fm)
       {
           device = d;
           DeviceInUI.PropertyChanged += new PropertyChangedEventHandler(box_PropertyChanged);
           box = (DeviceDataBox_KSJKSD)DeviceInUI.Value;
       ...
       {
           values.Clear();
           values.Add(box.DoseNow);
           updateChart(values);

           valueLT.getValueTextBlock().Text = box.DoseNow;
